...
class Seq2SeqPackedAttention(nn.Module):
    ...
    def forward(self, src, src_len, trg, teacher_forcing_ratio=0.5, is_inference=False):
        if is_inference:
            ...
            for t in range(1, src_len_):
                ...
                output, hidden, attention = self.decoder(
                    input_, hidden, encoder_outputs, mask)
                ...
                # find the maximum index of dimension 1, which is output_dim
            ...
            # outputs for predicting the word, attentions to see which word is important
        ...
        # trg: [trg len, batch size]
            ...
            # because decoder decodes each step....let's memorize the attention done in each step....
            attentions = torch.zeros(
                trg_len, batch_size, src.shape[0]).to(self.device)
